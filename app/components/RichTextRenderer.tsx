...
const RichTextRenderer = ({ content }: RichTextRendererProps) => {
    ...
    switch (node.type) {
      case "heading":
        ...
        const headingClasses: Record<number, string> = {
          1: "text-2xl text-left font-bold tracking-wider uppercase border-l-4 border-uzGreen px-2 mt-10",
      ...
      case "paragraph":
        return (
          <p
            key={index}
            className="text-lg mb-10 mt-10 leading-loose max-w-full"
          >
            {node.children.map((child: any, idx: number) =>
              renderNode(child, idx)
            )}
      ...
      case "list":
        ...
        return (
          <ListTag
            key={index}
            className="text-lg list-disc m-6 leading-loose max-w-full"
          >
            {node.children.map((child: any, idx: number) =>
              renderNode(child, idx)
            )}
      ...
      case "list-item":
        return (
          <li key={index} className="mb-6 text-lg leading-loose max-w-full">
            {node.children.map((child: any, idx: number) =>
              renderNode(child, idx)
            )}
          </li>
  ...
  return (
    <div className="max-w-full leading-loose text-lg">
